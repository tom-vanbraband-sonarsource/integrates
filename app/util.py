...
def cvss2_temporal(severity, basescore):
...
def cvss2_basescore(severity, parameters):
...
def cvss2_environment(severity, parameters):
    exploitabilty = parameters['exploitability_factor'] * severity['accessComplexity'] * \
        severity['authentication'] * severity['accessVector']
...
def calc_cvss_temporal(severity, basescore):
    cvss_temporal = cvss2_temporal(severity, basescore)
    return cvss_temporal


def calc_cvss_basescore(severity, parameters):
    cvss_basescore = cvss2_basescore(severity, parameters)
    return cvss_basescore


def calc_cvss_enviroment(severity, parameters):
    cvss_environment = cvss2_environment(severity, parameters)
