...
def __send_mail(template_name, email_to, context, tags):
...
def send_mail_new_vulnerabilities(email_to, context):
    __send_mail('newvulnerabilitiesintegrates', email_to, context=context, tags=VULNERABILITIES_TAG)


def send_mail_change_finding(email_to, context):
    __send_mail('changefindingintegrates', email_to, context=context, tags=GENERAL_TAG)


def send_mail_new_user(email_to, context):
    __send_mail('userfindingintegrates', email_to, context=context, tags=GENERAL_TAG)


def send_mail_delete_finding(email_to, context):
    __send_mail('deletefindingintegrates', email_to, context=context, tags=GENERAL_TAG)
...
def send_mail_verified_finding(email_to, context):
    __send_mail('verifiedfindingintegrates', email_to, context=context, tags=VERIFY_TAG)

def send_mail_new_remediated(email_to, context):
    __send_mail('newremediatefindingintegrates', email_to, context=context, tags=GENERAL_TAG)

def send_mail_add_access(email_to, context):
    __send_mail('addaccessintegrates', email_to, context=context, tags=GENERAL_TAG)

def send_mail_age_finding(email_to, context):
    __send_mail('agefindingintegrates', email_to, context=context, tags=AGE_TAG)

def send_mail_age_kb_finding(email_to, context):
    __send_mail('agekbfindingintegrates', email_to, context=context, tags=AGE_TAG)

def send_mail_delete_draft(email_to, context):
    __send_mail('deletedraftintegrates', email_to, context=context, tags=GENERAL_TAG)

def send_mail_new_releases(email_to, context):
    __send_mail('newreleasesintegrates', email_to, context=context, tags=GENERAL_TAG)

def send_mail_access_granted(email_to, context):
    __send_mail('accessgrantedintegrates', email_to, context=context, tags=GENERAL_TAG)

def send_mail_continuous_report(email_to, context):
    __send_mail('continuousreport', email_to, context=context, tags=GENERAL_TAG)

def send_mail_integrates_new_version(email_to, context):
    __send_mail('integratesnewversion', email_to, context=context, tags=GENERAL_TAG)

def send_mail_repositories(email_to, context):
    __send_mail('repositoriesintegrates', email_to, context=context, tags=GENERAL_TAG)

def send_mail_unsolved_events(email_to, context):
    __send_mail('unsolvedevents', email_to, context=context, tags=GENERAL_TAG)
