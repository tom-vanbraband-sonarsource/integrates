...
class ViewTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not os.path.exists(profile_path):
            ...
            s3_res.Bucket(s3_bucket).download_file(
    ...
    def test_05_finding(self):
        ...
        assert('The forms in the application allow the injection and execution ' +
               'of code in Javascript, making possible for an attacker to'
    ...
    def test_10_comments(self):
        ...
        WebDriverWait(
            selenium, self.delay).until(
                exp_cond.presence_of_element_located(
                    (By.XPATH,
                     "//*[contains(text(), 'Validations were run against 5 of')]")))
        time.sleep(3)
        selenium.save_screenshot(SCR_PATH + '10-04-comments.png')
        assert 'Validations were run against 5 of the 10 reported fields' \
            in selenium.page_source
