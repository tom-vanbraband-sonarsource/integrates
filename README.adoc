...
* The file _**.envrc**_ contains all the environment variables for proper
  execution of Integrates, for security reasons the file is not in the
  repository. For a copy, please contact _jrestrepo@fluidattacks.com_.

* It is necessary to add the following line: _**eval"$(direnv hook bash)"**_ in
  the end of file that is displayed when executing the command:
...
* Locate the file _.envrc_ in the main Integrates folder, that is, in the path
  /var/www/integrates and then execute the following command:
...
* To confirm that variables were correctly loaded, run the following command
  and verify that the variables that are in _.envrc_ file are displayed.
